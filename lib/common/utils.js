...
Utils.formatAmount = function(atoms, unit, opts) {
  var UNITS = {
    dcr: {
      toAtoms: 100000000,
      maxDecimals: 6,
  ...
  var amount = (atoms / u.toAtoms).toFixed(u.maxDecimals);
  return addSeparators(amount, opts.thousandsSeparator || ',', opts.decimalSeparator || '.', u.minDecimals);
};

Utils.formatAmountInDcr = function(amount) {
  return Utils.formatAmount(amount, 'dcr', {
    minDecimals: 8,
    maxDecimals: 8,
  }) + 'dcr';
};
...
Utils.formatUtxos = function(utxos) {
  ...
  return _.map([].concat(utxos), function(i) {
    var amount = Utils.formatAmountInDcr(i.atoms);
